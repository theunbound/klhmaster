...
Here, the $/gamma$jet MC set has been composited from five different MC sets with successively higher minimum cuts on $p_T$\footnote{ More in appendix}. We combine them by picking those events from a given set that have the sum of reconstructed $p_T$ greater than 2.3 times the initial low cut for the set, and less than 2.3 times the initial low cut for the next set. We choose a cut above the initial value to clear their turn--on curve.
